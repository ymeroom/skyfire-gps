...
class SkyFireEngine {
   ...
   * @param {string} [params.locationName=''] 當前地點名稱
   * @param {Object} [params.customWeights] 自訂微調權重
  ...
  static calculate(params) {
    ...
    let moistureScore = 0;
    if (precipProb > 50) {
      moistureScore = -Math.min(25, (precipProb - 50) * 0.6);
  ...
  static getRatingLevel(score) {
    if (score >= 82) {
      return {
        ...
        bgGradient: 'linear-gradient(135deg, rgba(255,51,102,0.35) 0%, rgba(255,153,0,0.25) 100%)'
      };
    } else if (score >= 68) {
      return {
        level: 'GREAT',
  ...
  static generateDiagnostics(data) {
    ...
    const direction = type === 'sunset' ? '西方地平線' : '東方地平線';
    ...
    if (lowCloud <= 25 && horizonClearance >= 70) {
      list.push({
        ...
        status: 'optimal',
        desc: `低雲遮蔽極低 (${lowCloud}%)，透光窗達 ${horizonClearance}%，陽光無阻直射雲底！`
      });
    } else if (lowCloud > 60) {
      list.push({
        label: `遠方${direction}透光窗`,
  ...
  static getPhotographerAdvice(score, type = 'sunset') {
    ...
    if (score >= 70) {
      ...
      return {
        ...
        lens: '廣角至中望遠，適合捕捉地景夜景、橋樑車軌與山巒雲海',
