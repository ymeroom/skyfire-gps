/**
 * SolarCalc - 全球通用精確天文太陽計算庫
 * 支援全球任意經緯度與日期的太陽位置、日出日落、黃金時刻、藍調時刻及太陽方位角
 */
...
class SolarCalc {
   ...
   * 計算特定經緯度與日期的太陽時刻表
   * @param {Date} date 
   * @param {number} lat 緯度 (-90 到 90)
   * @param {number} lng 經度 (-180 到 180)
   */
  static getTimes(date = new Date(), lat = this.DEFAULT_LOCATION.lat, lng = this.DEFAULT_LOCATION.lng) {
    ...
    // 晚霞火燒雲窗口：日落前 15 分鐘 至 日落後 30 分鐘 (特別是日落後 10-25 分漫天火燒最烈)
    if (result.sunset) {
   ...
   */
  static getPosition(date = new Date(), lat = this.DEFAULT_LOCATION.lat, lng = this.DEFAULT_LOCATION.lng) {
    const d = this.toDays(date);
    const lw = -lng * this.rad;
    const phi = lat * this.rad;
