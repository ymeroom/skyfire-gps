/**
 * test-ray-path-model.js - 測試分層光路帶狀取樣模型 (Layered Ray-Path Sampling)
 */

const assert = require('assert');
const WeatherService = require('../js/weather-service.js');

console.log('--- 🧪 測試 7: 分層光路帶狀取樣模型 ---');
...
// ----------------------------------------------------------------
const TEST_LAT = 25.0330;
const TEST_LNG = 121.5654;
const TEST_NAME = '光路模型測試點';
...
const parsedBlocked = WeatherService.processRawData(clearLocal, TEST_LAT, TEST_LNG, TEST_NAME, blockedHighBand, blockedHighBand);
...
const parsedClear = WeatherService.processRawData(clearLocal, TEST_LAT, TEST_LNG, TEST_NAME, allClear, allClear);
...
//    而且山體本身就會直接遮斷射線。
//    註：Open-Meteo 的 elevation 對盆地地形也回傳 0，因此「高度 0」只能
...
const parsedTerrain = WeatherService.processRawData(clearLocal, TEST_LAT, TEST_LNG, TEST_NAME, mountainPath, mountainPath);
...
console.log('🎉 分層光路帶狀取樣模型測試全數 PASS!\n');
